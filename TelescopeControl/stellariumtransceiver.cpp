...
#define DEG_PER_MIL 0.1978021978
    ...
    QObject(parent)
{
    ...
    m_tcpServer.listen(QHostAddress::LocalHost, 10001);
}
...
void StellariumTransceiver::slotReadyRead() {
    ...
    qDebug() << "ALT:" << ALT;
    qDebug() << "AZ:" << AZ;
    qDebug() << "prevALT:" << prevAlt;
    qDebug() << "prevAZ:" << prevAz;
    ...
    rot = (AZ - prevAz) / DEG_PER_MIL;
    asc = (ALT - prevAlt) / DEG_PER_MIL;
    printf("rot = %d asc = %d\n", rot, asc);
    snprintf(command, 256, "curl 192.168.4.1/motor?rotate=%d\\&ascencion=%d", rot >= 0 ? rot : -rot, asc >= 0 ? asc : -asc);
    if (rot >= 0)
        strcat(command, "\\&dir1=0");
    else
        strcat(command, "\\&dir1=1");
    if (asc >= 0)
        strcat(command, "\\&dir2=0");
    else
        strcat(command, "\\&dir2=1");
